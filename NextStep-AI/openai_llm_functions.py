...
def test_message_career_assistant(user_input):
    # Debug line to know when OpenAI API is called
    print("Calling test_message_career_assistant function")

    # Add user's last message to the chat array (context)
    test_prompt_career_assistant_history.append(
        {
            "role": "user",
            "content": user_input
        }
    ...
    response = openai_client.chat.completions.create(
        model="gpt-4o-mini",
        messages=test_prompt_career_assistant_history
    )

    # Append newest response to the chat array (context)
    message = response.choices[0].message
    test_prompt_career_assistant_history.append(message)
    ...
    json_end = message.content.rfind(']') + 1
    suggestions_json = message.content[json_start:json_end]
    print("Completed parse on JSON")

    # Reformat the response into a TTS friendly response
    tts_response = test_reformat_for_tts(message.content)
    ...
    # Debug line to know when leaving the function
    print("Leaving test_message_career_assistant function")

    # Return the JSON and TTS response
    return suggestions_json, tts_response


def test_reformat_for_tts(structured_response):
    # Debug line to know when OpenAI API is called
    print("Calling test_reformat_for_tts function")

    # Generate next 'TTS Assistant' response by giving ChatGPT the latest response
    response = openai_client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": test_prompt_tts_assistant},
            {"role": "user", "content": f"Please reformat this for TTS: {structured_response}"}
        ]
    )

    # Debug line to know when leaving the function
    print("Leaving test_reformat_for_tts function")
